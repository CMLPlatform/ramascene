...
4:merging these objects as one back together and writing to file
Note: this script is not used by ramascene due to technical limitations of viz. library, see geo_dissolve_by_level.py
"""
...
def append_columns_and_match(df, rest_dct,normal_dct,cont_dct,tot_dct):
    ...
    return rest_df, cont_df,countries_df, total_df
...
def dissolve_rest(df):
    ...
    import fiona
    for name in fiona.listlayers(FNAME_GEO):
        with fiona.open(FNAME_GEO, layer=name) as c:
            sink_schema = c.schema.copy()
            t = c.schema.copy()
            my_schema = (sink_schema)
            my_schema['properties']['ID'] = 'str'
    print(t)
    print(my_schema)
    ...
    try:
        ...
        rest_of.to_file(driver='GeoJSON', filename=r'./output/test.geojson')
...
def write(rest,cont,countries,total):
    ...
    final_df = gpd.GeoDataFrame(pd.concat([rest,cont,countries,total], ignore_index=True))

    del final_df['ADMIN']
    final_df = final_df.rename(columns={'ISO_A3': 'id'})
    print(final_df)
    try:
        final_df.to_file(driver='GeoJSON', filename=r'./output/test.geojson')
    except Exception as e:
        print(e)
        print("***Failed creating file. File already exists***")
